...
# -*- coding: utf-8 -*-
"""
TransReID - VeRi-776 launcher (ViT only)
...
[2025-08-30 | Hang Zhang] Parity with DeiT: ensure pretrained dir exists; check pretrained file.
[2025-08-30 | Hang Zhang] Changed default pretrained weight to jx_vit_base_p16_224-80ecf9dd.pth.
"""
...
def main():
    parser = argparse.ArgumentParser(description="TransReID VeRi-776 launcher (ViT only).")
    ...
    parser.add_argument("--config", type=str, default="configs/VeRi/vit_transreid_stride.yml", help="ViT config")

    # [2025-08-30 | Hang Zhang] Changed default pretrained weight to match local file.
    parser.add_argument("--pretrained", type=str,
                        default="pretrained/jx_vit_base_p16_224-80ecf9dd.pth",
                        help="ViT ImageNet weight")
    ...
    out_run = Path("logs/veri776_vit_run").resolve()
    out_test = Path("logs/veri776_vit_test").resolve()
    train_log = out_run / "train.log"
    test_log = out_test / "test_all.log"

    out_run.mkdir(parents=True, exist_ok=True)
    out_test.mkdir(parents=True, exist_ok=True)
    Path("pretrained").mkdir(parents=True, exist_ok=True)  # ensure dir exists
    ...
    print("===== Begin per-epoch evaluation (ViT) =====")
