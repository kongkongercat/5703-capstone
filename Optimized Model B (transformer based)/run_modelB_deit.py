...
[2025-09-16 | Hang Zhang] **Fix resume sorting** (numeric sort for transformer_*.pth) and
                          **Auto-append seed** to tag when SOLVER.SEED is provided.
"""
...
# ---------- FIXED: numeric sort for latest checkpoint ----------
_num_re = re.compile(r"transformer_(\d+)\.pth$")

def find_latest_ckpt(run_dir: Path) -> Path | None:
    """Return newest transformer_*.pth in run_dir, or None if absent (numeric sort)."""
...
# -------- Environment info printer --------
def print_env_info() -> None:
    """Print Python/PyTorch/CUDA/MPS/GPU/CPU/OS to stdout (and train.log via tee)."""
    try:
        import platform
        import torch  # type: ignore
        ...
        try:
            cpu_name = platform.processor() or f"{os.cpu_count()} logical cores"
...
def main():
    ...
    # passthrough YACS overrides; handle TAG/OUTPUT_DIR specially
    parser.add_argument(
        "--opts",
    ...
    # ---- Parse --opts into pairs; capture OUTPUT_DIR/TAG/SEED overrides, forward the rest ----
    ...
    # NEW: auto-append seed suffix if provided and not already present
    ...
    print(f"Config            : {cfg}")
    print(f"Tag               : {tag}")
    print(f"Seed              : {seed_from_opts if seed_from_opts is not None else 'None'}")
