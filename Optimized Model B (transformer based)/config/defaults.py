...
# [2025-11-04 | Hang Zhang] **Add CLIP switch framework (no behavior change by default)**:
#                           - MODEL.USE_CLIP (bool), MODEL.CLIP_IMPL in {"open_clip","hf"}
#                           - MODEL.CLIP_FUSION in {"minimal","quickwin"} (only for open_clip)
#                           - OpenCLIP config keys: MODEL.CLIP_BACKBONE, MODEL.CLIP_PRETRAIN
#                           - Keep existing TinyCLIP (HF) keys; default USE_CLIP=False preserves baseline.
# -----------------------------------------------------------------------------
...
_C.LOSS.SUPCON.DECAY_TYPE = "const"
_C.LOSS.SUPCON.DECAY_START = 9999
_C.LOSS.SUPCON.DECAY_END = 10000

# ---- SupCon auto-search grid (legacy; disabled by default) -------------------
# [Usage scenario]
#   - Originally used in early B1 experiments for hyperparameter grid search
#     over SupCon weight (W) and temperature (T).
#   - Only takes effect if the training script explicitly iterates over
#     LOSS.SUPCON.SEARCH.W/T to spawn multiple runs (e.g., in train_net.py).
#   - Has **no impact** on normal training unless such a loop is implemented.
#   - Default Model B / B2 pipeline ignores this field (kept for backward
#     compatibility with B1 YAMLs and automated sweep scripts).
...
_C.LOSS.PHASED.BOUNDARIES = [30, 60]
...
_C.LOSS.BRIGHTNESS.ENABLE = False
_C.LOSS.BRIGHTNESS.THRESH = 0.35
_C.LOSS.BRIGHTNESS.K = 0.08

# -----------------------------------------------------------------------------
# CLIP switches (unified; default OFF preserves baseline)
# -----------------------------------------------------------------------------
_C.MODEL.USE_CLIP = False
_C.MODEL.CLIP_IMPL = "open_clip"     # {"open_clip","hf"}
_C.MODEL.CLIP_FUSION = "minimal"     # {"minimal","quickwin"}
...
_C.MODEL.CLIP_USE_AFEM = True
_C.MODEL.CLIP_USE_SEM_REFINE = True
